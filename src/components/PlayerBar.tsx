...
export function PlayerBar({ name, color, capturedPieces, advantage, isActive, time, className }: PlayerBarProps) {
  ...
  return (
    <div className={cn("flex justify-between items-center w-full bg-[#1A1A1E]/80 backdrop-blur-md px-4 py-2 sm:px-6 sm:py-3 rounded-xl border transition-colors", isActive ? "border-purple-500/50 shadow-[0_0_15px_rgba(168,85,247,0.15)]" : "border-white/5", className)}>
        ...
        <div className="flex flex-col">
          ...
          <div className="flex items-center gap-1 min-h-[1.5rem]">
            {capturedPieces.map((p, i) => (
              <span key={i} className="text-sm sm:text-base drop-shadow-md" style={{
                color: color === 'w' ? '#000' : '#fff',
                WebkitTextStroke: "1px rgba(100,100,100,0.5)",
              }}>
                {PIECE_SYMBOLS[settings.pieceStyle as 'solid' | 'flat'][p]}
              </span>
            ))}
            {advantage > 0 && (
